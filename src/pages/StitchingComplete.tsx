...
import { stitchingColumnsComplete } from "@/table-columns/stitching-complete-columns";

import { useState, useContext } from "react";

import { StitchingContext } from "@/context/StitchingContext";

export default function StitchingComplete() {
  const [searchTerm, setSearchTerm] = useState("");
  const { completedItems } = useContext(StitchingContext);

  const filteredStitchingData = completedItems.filter((item) =>
    ...
    <div className="min-h-screen p-6">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white shadow-md overflow-hidden">
        <DataTable
          ...
          columns={stitchingColumnsComplete}
          enablePagination
        />
      </div>
    </div>
